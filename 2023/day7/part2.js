...
class Hand {
    constructor(hand, bet) {
        ...
        for (var i = 0; i < sortedInstances.length; i++) {
            if (sortedInstances[i][0] == "J") {
                if (i == 0 && sortedInstances.length > 1) sortedInstances[1][1] += sortedInstances[i][1]
                else sortedInstances[0][1] += sortedInstances[i][1]
                sortedInstances.splice(i, 1)
            }
        }
        sortedInstances.sort((a, b) => b[1] - a[1])
        if (sortedInstances.length == 0)
            sortedInstances.push(["J", 5])
